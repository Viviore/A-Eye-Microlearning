...
import {
  SKILL_QUESTIONS,
  TOTAL_SKILL_QUESTIONS,
} from '@/data/quizQuestions';

// ─── Assessment Types ────────────────────────────────────────────────
export type SkillAnswer = "real" | "fake";
...
// ─── Score Calculator ────────────────────────────────────────────────
function calculateSkillScore(answers: AssessmentAnswers): {
  correctCount: number;
  skillPercentage: number;
} {
  let correctCount = 0;
  for (const sq of SKILL_QUESTIONS) {
    const userAnswer = answers[sq.id];
    if (userAnswer === sq.expected) {
      correctCount++;
    }
  }
  const skillPercentage = Math.round(
    (correctCount / TOTAL_SKILL_QUESTIONS) * 100
  );
  return { correctCount, skillPercentage };
}
...
interface GameState {
  ...
  preAssessmentResult: AssessmentResult | null;
  postAssessmentResult: AssessmentResult | null;

  completePreAssessment: (answers: AssessmentAnswers) => void;
  completePostAssessment: (answers: AssessmentAnswers) => void;

  // Legacy quiz score fields (kept for compatibility with results page)
...
export const useGameStore = create<GameState>()(
  persist(
    (set) => ({
      ...
      postAssessmentResult: null,

      completePreAssessment: (answers) =>
        set(() => {
          const { correctCount, skillPercentage } = calculateSkillScore(answers);
          const awareness = answers.q6 ?? 1;
          const confidence = answers.q7 ?? 1;
          ...
          return {
            ...
            preQuizScore: correctCount,
            preQuizAwareness: awareness,
            preQuizConfidence: confidence,
          };
        }),
      ...
      completePostAssessment: (answers) =>
        set(() => {
          ...
          return {
            ...
            postAssessmentResult: result,
            // Set legacy fields for backward compatibility
            postQuizScore: correctCount,
            postQuizAwareness: awareness,
            postQuizConfidence: confidence,
          };
        }),

      // ─── Legacy Quiz Score Fields ──────────────────────────────────
      preQuizScore: null,
