export function PostAuthorHeader({
  ...
  return (
    <div className="flex items-start md:items-center gap-4 md:gap-5 mb-6 md:mb-8 border-b-[4px] border-dashed border-[#0F172A]/30 pb-4 md:pb-6">
      <div 
        className={`w-14 h-14 md:w-16 md:h-16 ${avatarColor} border-[4px] border-[#0F172A] shadow-[4px_4px_0px_0px_#0F172A] flex items-center justify-center overflow-hidden shrink-0`}
      >
      ...
      <div>
        <h4 className="font-heading font-black text-2xl md:text-3xl leading-tight text-[#0F172A] uppercase tracking-wide">{authorName}</h4>
        <p className="text-[15px] md:text-base font-mono font-bold text-[#0F172A]/70 mt-1">
          {handle}
          {time && (
